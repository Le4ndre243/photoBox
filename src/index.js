(() => {
  ...
  function loadResource(uri) {
    return fetch(uri, FETCH_OPTIONS).then((r) => {
  ...
  function loadPicture(idPicture) {
    ...
    return fetch(url, FETCH_OPTIONS).then((r) => {
      if (!r.ok) return Promise.reject(new Error(`HTTP ${r.status}: ${r.statusText}`));
      return r.json();
    }).catch((err) => {
      if (err instanceof Error) {
  ...
  function loadPhotos(uri) {
    const url = uri != null ? uri : `${API_BASE_URL}/photos`;
    return loadResource(url);
  }
  function loadCategorie(photo) {
    var _a;
    const uri = (_a = photo.links) == null ? void 0 : _a.categorie;
    if (!uri) return Promise.reject(new Error("Pas de lien cat\xE9gorie pour cette photo"));
    return loadResource(uri);
  }
  function loadCommentaires(photo) {
    var _a;
    const uri = (_a = photo.links) == null ? void 0 : _a.commentaires;
    if (!uri) return Promise.reject(new Error("Pas de lien commentaires pour cette photo"));
    return loadResource(uri).then((result) => {
  ...
  function getTemplate(id) {
    ...
    return import_handlebars.default.compile(el.innerHTML);
  }
  ...
  function displayCategorie(categorie) {
    ...
    if (!container) return;
    container.textContent = (_a = categorie.libelle) != null ? _a : String(categorie);
  }
  ...
  function display_galerie(photos) {
    ...
    const template = import_handlebars2.default.compile(templateEl.innerHTML);
  ...
  function load(uri) {
    return loadPhotos(uri).then((collection) => {
      var _a, _b;
      state.photos = (_a = collection.data) != null ? _a : [];
      state.links = (_b = collection.links) != null ? _b : {};
      return state.photos;
    });
  }
  function next() {
  ...
  function prev() {
    if (!state.links.prev) return Promise.reject(new Error("Pas de page pr\xE9c\xE9dente"));
    return load(state.links.prev);
  }
  function first() {
    if (!state.links.first) return Promise.reject(new Error("Pas de premi\xE8re page"));
    return load(state.links.first);
  }
  function last() {
    if (!state.links.last) return Promise.reject(new Error("Pas de derni\xE8re page"));
  ...
  function getPicture(id) {
    loadPicture(id).then((photo) => {
      ...
      getCategorieForPhoto(photo).then(displayCategorie).catch((err) => {
        console.warn("Cat\xE9gorie non disponible:", err);
      });
      loadCommentaires(photo).then(displayCommentaires).catch((err) => {
  ...
  function attachGalerieClickHandlers() {
    const vignettes = document.querySelectorAll("#la_galerie [data-photo-id]");
    vignettes.forEach((el) => {
      el.addEventListener("click", () => {
        ...
        getPicture(id);
        (_a = document.querySelector("#la_photo")) == null ? void 0 : _a.scrollIntoView({ behavior: "smooth" });
      });
    });
  }
